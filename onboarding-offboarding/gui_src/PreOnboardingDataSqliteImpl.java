...
public class PreOnboardingDataSqliteImpl {
    ...
    public void ensureTablesExist() {
        ...
        try (Connection conn = connect();
            ...
            ensureCandidateColumns(conn);
            migrateLegacyCandidateData(conn);
    ...
    private void ensureCandidateColumns(Connection conn) throws Exception {
    ...
    private void migrateLegacyCandidateData(Connection conn) throws Exception {
...
// Removed test data
        ...
        PreparedStatement ps = conn.prepareStatement(
            "INSERT INTO candidates (candidate_id, contact_info, email, phone, name, skills, status) " +
            "VALUES (?,?,?,?,?,?,?)");
    ...
    public List<String[]> getAllCandidates() {
        ...
        String sql = "SELECT candidate_id, COALESCE(email, contact_info) AS email, name, skills, status, phone FROM candidates";
        try (Connection conn = connect();
             Statement stmt = conn.createStatement();
    ...
    public String[] getCandidateById(String candidateId) {
        String sql = "SELECT candidate_id, COALESCE(email, contact_info) AS email, name, skills, status, phone FROM candidates WHERE candidate_id = ?";
        try (Connection conn = connect();
    ...
    public void addCandidate(String id, String email, String name,
                            String phone, String skills) {

        String sql = "INSERT INTO candidates " +
                    "(candidate_id, contact_info, email, phone, name, skills, status) " +
                    "VALUES (?, ?, ?, ?, ?, ?, 'PENDING')";
    ...
    public void updateOnboardingStatus(String candidateId, String status) {
        if ("COMPLETED".equals(status) && !areAllTasksCompleted(candidateId)) {
            throw new IllegalStateException(
                "Cannot mark pre-onboarding as completed until all tasks for this candidate are completed."
            );
        }
        ...
        try (Connection conn = connect();
            ...
            if ("COMPLETED".equals(status)) {
                String[] c = getCandidateById(candidateId);
                if (c != null) {
                    // c = [candidate_id, email, name, skills, status, phone]
                    model.model.Candidate cand = new model.model.Candidate(
                        c[0], c[2], c[1], c[3], null, null, "ONBOARDING_IN_PROGRESS"
                    );
                    UnifiedDatabaseManager.migrateCandidateToEmployee(cand);
                }
            }
        } catch (Exception e) {
    ...
    public boolean areAllTasksCompleted(String candidateId) {
        String sql = "SELECT COUNT(*) AS total, " +
                     "SUM(CASE WHEN status = 'COMPLETED' THEN 1 ELSE 0 END) AS completed " +
                     "FROM pre_onboarding_tasks WHERE candidate_id = ?";
        try (Connection conn = connect();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, candidateId);
            ResultSet rs = ps.executeQuery();
            ...
            System.err.println("areAllTasksCompleted error: " + e.getMessage());
        }
        return false;
    }
