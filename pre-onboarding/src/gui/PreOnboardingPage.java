...
import data.PreOnboardingDataSqliteImpl;
...
public class PreOnboardingPage extends JPanel {
    ...
    private JPanel buildCandidateDetail(String[] c) {
        ...
        detailStatusBadge = createStatusBadge(c[6]);
        ...
        detailRoleLabel = new JLabel(c[4] + " · " + c[5]);
        ...
        grid.add(makeDetailField("Skills", c[4]));
        grid.add(makeDetailField("Source", c[5]));
        grid.add(makeDetailField("ID", c[0]));
        grid.add(makeDetailField("Resume", c[3]));
        grid.add(makeDetailField("Status", c[6]));
    ...
    private JPanel buildCandidateCard(String[] c) {
        // c = [id, contact_info, name, resume_path, skills, source, status]
        ...
        JLabel role = new JLabel(c[4] + " · " + c[5]); // optional (skills + source)
        ...
        JLabel badge = createStatusBadge(c[6]);
    ...
    private void showAddCandidateDialog() {
        ...
        JPanel form = new JPanel(new GridLayout(6, 2, 12, 10));
        form.setOpaque(false);

        JTextField fName    = new JTextField();
        JTextField fEmail   = new JTextField();
        JTextField fSkills  = new JTextField();
        JTextField fSource  = new JTextField();
        ...
        styleTextField(fEmail,  "Email");
        styleTextField(fSkills, "Skills");
        styleTextField(fSource, "Source");
        styleTextField(fResume, "Resume Path");
        ...
        form.add(makeLabel("Source"));    form.add(fSource);
        form.add(makeLabel("Resume"));    form.add(fResume);
        ...
        save.addActionListener(e -> {
            ...
            String source = fSource.getText().trim();
            String resume = fResume.getText().trim();
            ...
            db.addCandidate(id, email, name, resume, skills, source);
            db.updateOnboardingStatus(id, status);            dlg.dispose();
            loadData();
            selectCandidate(id);
        });
        btnRow.add(cancel);
